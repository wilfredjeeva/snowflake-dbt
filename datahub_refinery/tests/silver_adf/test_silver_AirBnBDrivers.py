import pytest
import supprt_file_silver
from supprt_file_silver import *
...
def test_recomputed_hash_matches_between_layers(sf_conn):
    ...
    if len(merged) != len(bronze_df) or len(merged) != len(silver_df):
        ...
        print("Merged LEngth:", len(merged), len(bronze_df),len(silver_df))
        print("Bronze Keys:",bronze_keys)
        print("Silver Keys:",silver_keys)
        print("Extra Keys:",extra_in_silver)
        assert False, (
...
@pytest.mark.smoke
def test_bronze_silver_count(sf_conn):
    # Use quoted identifier for case-sensitive landing table
    bronze_table = f'{bronze_db}.{airBnB_schema}.{airBnB_drivers_table}'
    silver_table = f"{silver_db}.{airBnB_schema}.{airbnb_silver_table}"
    compare_row_counts_activeRows(sf_conn, bronze_table, silver_table)
